...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'approve') {
      ...
      if (result.invoice_id) {
        db.prepare("UPDATE invoices SET match_status = 'unmatched' WHERE id = ?").run(result.invoice_id);
      }
      if (result.bank_transaction_id) {
        db.prepare("UPDATE bank_transactions SET match_status = 'unmatched' WHERE id = ?").run(result.bank_transaction_id);
      }
      if (result.payment_transaction_id) {
        db.prepare("UPDATE payment_transactions SET match_status = 'unmatched' WHERE id = ?").run(result.payment_transaction_id);
      }
    }
    ...
    db.prepare(`INSERT INTO audit_logs (id, user_id, user_name, action, record_type, record_id, old_value, new_value, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      ...
      action === 'approve' ? 'Approved Match' : 'Rejected Match',
